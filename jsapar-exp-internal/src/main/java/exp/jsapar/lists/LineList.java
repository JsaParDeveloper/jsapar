...
/**
 * Implements a LineList, which contains two types of interfaces: List and
 * FilterableList. The List implementation is used to interface to the external
 * world and make the LineList modifiable. The FiterableList implementation is
 * used to interface to the external world for filtering purposes only, and not
 * giving the user the control to change anything in the filtered list. The
 * filtered results can be used by the user to get knowledge of the elements
 * contained within the complete list. The information received from the
 * filtered list can be used to change the complete list.
 * 
 * TODO maybe we should implement it like this: the complete list and filtered
 * list are both synchronized to each other within ONE class, but have separate
 * package-private classes of their own. This way we can have one public list,
 * the CELLLIST, and two package-private sub lists ALLCELLLIST and
 * FILTERCELLLIST. The ALLCELLLIST and FILTERCELLLIST must be internally
 * synchronized to keep both lists accurate. Next, the normal iterators must be
 * overridden (iterator() and listIterator()) because the normal iterators in
 * the lists cannot be used, due to the fact that the hasNext(), next() and
 * remove() methods DO have effect on both the lists. The remove() method in the
 * iterator for the ALLCELLLIST has influence on the elements in the FILTERCELLLIST
 * and vice versa.  
 * 
